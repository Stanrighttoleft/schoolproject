...
function YY_checkform() {
  ...
  for (var i = 1; i < args.length; i = i + 4) {
    ...
    var myObj = MM_findObj(args[i].replace(/\[\d+\]/gi, ""));
    myV = myObj.value;
    if (
      myObj.type == "text" ||
      myObj.type == "password" ||
      ...
      if (myV.length > 0 && args[i + 2] == 1) {
        ...
        // check this 2
        var myObj1 = MM_findObj(args[i + 1].replace(/\[\d+\]/gi, ""));
        if (myObj1.length)
          myObj1 = myObj1[args[i + 1].replace(/(.*\[)|(\].*)/gi, "")];
        if (!myObj1.checked) {
          addErr = true;
        }
      ...
      } else if (myV.length >= 0 && args[i + 2] == 7) {
        // the same 允许栏位空白比对
        addErr = false;
        var myObj1 = MM_findObj(args[i + 1]);
        if (myV != myObj1.value) {
